...
public partial class KnowledgeBase_add : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
			...
			gblData.FillDropdown(ddlknowledgecategoryid, "tbl_knowledgecategory", "knowledgecategory_category", "knowledgecategory_knowledgecategoryid", "", "knowledgecategory_category");
			//FillDropDown_END
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Knowledge Base";
        }
        else
        {
            lblPageTitle.Text = "Edit Knowledge Base";
        }
    ...
    protected void btnSaveAndView_Click(object sender, EventArgs e)
    {
        if (SaveData(false) > 0)
		{
        	Response.Redirect("~/KnowledgeBase/view.aspx");
		}
    }
    private int SaveData(bool isclose)
    {
        ...
        if (id > 0)
        {
			...
			//SaveSubTable_END
            //SaveFile_START
			mfuknowlegbasedocument.Save(id);
			//SaveFile_END
