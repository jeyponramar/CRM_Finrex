...
/// </summary>
public static class UpdatableGrid
{
    public static string _RootNode = "/setting/updatablegrid";
    ...
    public static void bindGrid(Control _form, Literal lt, string strNodeAttrName)
    {
        bindGrid(_form, lt, strNodeAttrName, Common.GetModuleName());
    }
    public static void bindGrid(Control _form, Literal lt,string strNodAttrName, string strModuleName)
    {        
        //XmlDocument doc = new XmlDocument();//[@m='stockinidetails']
       string strRootNode = _RootNode + "[@name='" + strNodAttrName + "']"; 
        //doc.Load(XMLNodeBinder.getXMLFilePath(strModuleName));        
       ...
       string strQuery = XMLNodeBinder.getSingleNodeText(strRootNode + "/query");
        string strHtmlScript = XmlEncoder(XMLNodeBinder.getSingleNodeText(strRootNode + "/htmlscript"));
        strQuery = XMLQueryBuilder.bulidXMLQuery(_form, strQuery);
        if (strQuery != "")
        {
            lt.Text = bindGrid(DbTable.ExecuteSelect(strQuery), strHtmlScript, arrGridColumns, arrGridColumnsHeader);
        }
        else
        {
    ...
    private static string XmlEncoder(string val)
    {
        return val.Replace("&lt;", "<").Replace("&gt;", ">");
    }
    public static string bindGrid(DataTable dttbl,string htmlscript, Array _arrColumns, Array _arrHeaderColumns)
    {
        ...
        if (GlobalUtilities.IsValidaTable(dttbl))
        {
            ...
            for (int i = 0; i < dttbl.Rows.Count; i++)
            {
                ...
                for (int j = 0; j < _arrColumns.Length && _arrColumns.Length == _arrHeaderColumns.Length; j++)
                {
                    ...
                    strHTMLScriptWithData = (strHTMLScriptWithData == "") ? htmlscript.Replace("$" + strColumnName + "$", strColumnValue) : strHTMLScriptWithData.Replace("$" + strColumnName + "$", strColumnValue);
                }
                strHTMLScriptWithData = strHTMLScriptWithData.Replace("$Count$", (i + 1).ToString());
                html.Append(strHTMLScriptWithData);
                html.Append("</tr>");
            }
            html.Append("</table>");
            html.Append("</td>");
