...
public partial class ExpenseType_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_expensetype", "expensetypeid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                //Populate_END
                ViewState["OldLedgerName"] = txtexpensetype.Text;
            }
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Expense Type";
        }
        else
        {
            lblPageTitle.Text = "Edit Expense Type";
        }
    ...
    {
        SaveData(false);
        Response.Redirect("~/ExpenseType/view.aspx");
    }
    private int SaveData(bool isclose)
    {
        ...
        int id = 0;
        int ledgerId = Accounts.SaveLedger(txtexpensetype.Text, LedgerGroup.IndirectExpense, 
            GlobalUtilities.ConvertToString(ViewState["OldLedgerName"]), LedgerType.Expense);
        if (ledgerId == 0)
        {
