...
public partial class DailyEmailSetting_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_dailyemailsetting", "dailyemailsettingid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Daily Email Setting";
        }
        else
        {
            lblPageTitle.Text = "Edit Daily Email Setting";
        }
    ...
    private int SaveData(bool isclose)
    {
        ...
        int time = GlobalUtilities.ConvertToInt(txttime.Text.ToLower().Replace("am", "").Replace("pm", "").Trim());
        if (time == 0)
        {
        ...
        if (txttime.Text.ToLower().Contains("am") || txttime.Text.ToLower().Contains("pm"))
        {
        }
        else
        {
            lblMessage.Text = "Please mention AM/PM in Time!";
            lblMessage.Visible = true;
            return 0;
        }
    ...
    protected void chkIsEmail_Click(object sender, EventArgs e)
    {
        if (chkisemail.Checked)
        {
            txtemailbody.CssClass = "htmleditor";
        }
        else
        {
            txtemailbody.CssClass = "textarea"; 
        }
    }
}
