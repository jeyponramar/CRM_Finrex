...
public partial class ServicePlan_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_serviceplan", "serviceplanid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Service Plan";
        }
        else
        {
            lblPageTitle.Text = "Edit Service Plan";
        }
    ...
    private int SaveData(bool isclose)
    {
        ...
        if (id > 0)
        {
			...
			mcserviceplanservices.Save(id);
			mcserviceplansoftwares.Save(id);
			//SaveSubTable_END
            //SaveFile_START
			//SaveFile_END
    ...
    private void SaveServiceType(int id)
    {
        string query = "";
        query = "select * from tbl_servicetype where servicetype_serviceplanid="+id;
        Hashtable hstbl = new Hashtable();
        hstbl.Add("servicetype", txtplanname.Text);
        hstbl.Add("isserviceplan", "1");
        hstbl.Add("serviceplanid", id);
        InsertUpdate obj = new InsertUpdate();
        DataRow dr = DbTable.ExecuteSelectRow(query);
        int servicetypeId = 0;
        if (dr == null)
        {
            servicetypeId = obj.InsertData(hstbl, "tbl_servicetype");
        }
        else
        {
            servicetypeId = obj.UpdateData(hstbl, "tbl_servicetype", GlobalUtilities.ConvertToInt(dr["servicetype_servicetypeid"]));
        }
    }
