...
public partial class DailyLMEMetalRate_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_dailylmemetalrate", "dailylmemetalrateid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Daily LME Metal Rate";
        }
        else
        {
            lblPageTitle.Text = "Edit Daily LME Metal Rate";
        }
    ...
    private int SaveData(bool isclose)
    {
        ...
        string sqldate = GlobalUtilities.ConvertToSqlDate(GlobalUtilities.ConvertToDateFromTextBox(txtdate.Text));
        int metalId = GlobalUtilities.ConvertToInt(txtmetalid.Text);
        string query = "";
        query = @"select * from tbl_dailylmemetalrate where dailylmemetalrate_metalid=" + metalId + " and cast(dailylmemetalrate_date as date)=cast('" + sqldate + "' as date)";
        if (Request.QueryString["id"] != null)
        {
            query += " and dailylmemetalrate_dailylmemetalrateid<>" + GetId();
        }
        DataRow dr = DbTable.ExecuteSelectRow(query);
        if (dr != null)
        {
            lblMessage.Text = "Rate already exists!";
