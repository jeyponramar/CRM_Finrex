...
public partial class FinDocDepartment_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_findocdepartment", "findocdepartmentid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add FinDoc Department";
        }
        else
        {
            lblPageTitle.Text = "Edit FinDoc Department";
        }
    ...
    private int SaveData(bool isclose)
    {
        ...
        string query = @"select * from tbl_findocdepartment where findocdepartment_clientid=@clientid 
                        and findocdepartment_departmentname=@departmentname";
        Hashtable hstblParam = new Hashtable();
        hstblParam.Add("clientid", clientid);
        hstblParam.Add("departmentname", txtdepartmentname.Text);
        InsertUpdate obj = new InsertUpdate();
        DataRow dr = obj.ExecuteSelectRow(query, hstblParam);
        if (dr != null)
        {
            lblMessage.Text = "Duplicate department name not allowed.";
