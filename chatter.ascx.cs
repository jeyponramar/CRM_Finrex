...
public partial class chatter : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Page.Form.Attributes.Add("folder", "share");
        if (!IsPostBack)
        {
            txtTargetId.Text = Request.QueryString["id"];
            BindShare();
        }
    }
    public string Module
    {
        set
        {
            txtTargetModule.Text = value;
        }
        get
        {
            return txtTargetModule.Text;
        }
    }
    private void BindShare()
    {
        string query = "select * from tbl_share s " +
                        ...
                        "left join tbl_user u2 on u2.user_userid = sc.sharecomment_createdby " +
                        "where share_targetmodule='" + Module + "' AND share_targetid=" + GlobalUtilities.ConvertToInt(Request.QueryString["id"]) +
                        "order by share_shareid desc";
        ...
        while (i < dttbl.Rows.Count)
        {
            ...
            string firstName = Convert.ToString(dttbl.Rows[i]["user_fullname"]);
            string photoPath = "../images/user/thumb/" + userid + ".jpg";
            ...
            {
                photoPath = "../images/user/thumb/default.png";
            }
            if (attachment != "")
            {
                ...
                for (int j = 0; j < arrfiles.Length; j++)
                {
                    ...
                    if (arrf.Length > 1)
                    {
                        ...
                        {
                            attachmentHtml += "<a href='../download.aspx?f=upload/share/" + actualFileName + "' target='_blank' style='color:#444444;text-decoration:none;'>" +
            ...
            if (shareid != prevShareId)
            {
                ...
                html.Append("<tr>" +
                               "<td width='70' class='valign'><img src='" + photoPath + "' width='25'/></td>" + 
            ...
            while (i < dttbl.Rows.Count)
            {
                ...
                firstName = Convert.ToString(dttbl.Rows[i]["user_fullname1"]);
                photoPath = "../images/user/thumb/" + userid + ".jpg";
