...
public partial class Followups : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            //}
            TextBox txtemployeeid = (TextBox)Parent.FindControl("txtemployeeid");
            string strTeleCallerId = "0";
            if (txtemployeeid != null)
            {
                strTeleCallerId = txtemployeeid.Text;
            }
            grid.AddUrl = "#followups/add.aspx?m=" + Module.Replace("followups_", "") + "&mid=" + Request.QueryString["id"] + "&telecallerId=" + strTeleCallerId;
    ...
    public bool grid_gridDataRowBind(DataTable dttbl, int i, StringBuilder html)
    {
        ...
        int telecallerId = GlobalUtilities.ConvertToInt(dttbl.Rows[i]["followups_employeeid"]);
        if (telecallerId > 0)
        {
            telecallerName = Common.GetOneColumnData("tbl_employee", telecallerId, "employeename");
        }
                        ...
                        "<td><table width='100%'>" +
                            ...
                            "<tr><td><table><tr>" +
                               ...
                               "<td style='padding-left:20px;'>Date :</td><td class='bold'>" +
                                                GlobalUtilities.ConvertToDateTime(dttbl.Rows[i]["followups_date"]) +
                                            " " + GlobalUtilities.ConvertToDate(dttbl.Rows[i]["followups_time"]) + "</td>" +
                               "<td style='padding-left:20px;'>Status :</td><td class='bold'>" + Convert.ToString(dttbl.Rows[i]["followupstatus_status"]) + "</td>" +
                               "<td style='padding-left:20px;'>Assigned To :</td><td class='bold'>" + telecallerName + "</td>" +
                               "</tr></table></td>" +
                            "</tr>" +
