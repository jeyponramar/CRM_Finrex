...
{
    GlobalData gblData = new GlobalData("tbl_task", "taskid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                //Populate_END
                int taskTypeId = GlobalUtilities.ConvertToInt(txttasktypeid.Text);
                if (taskTypeId < 5)
                {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Task";
        }
        else
        {
            lblPageTitle.Text = "Edit Task";
        }
    ...
    protected void btnSaveAndView_Click(object sender, EventArgs e)
    {
        if (SaveData(false) > 0)
		{
        	Response.Redirect("~/Task/view.aspx");
		}
    }
    private int SaveData(bool isclose)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            gblData.AddExtraValues("statusid", 1);
        }
        ...
        if (id > 0)
        {
            ...
            task.UpdateTaskParentModule(id, GlobalUtilities.ConvertToInt(txttasktypeid.Text));
            //SaveSubTable_START
	...
	{
        SaveData(false);
        Common.UpdateStatus(Page, Convert.ToInt32(Status.Hold), GetId());
	}
	protected void btnclose_Click(object sender, EventArgs e)
	{
        SaveData(false);
        Common.UpdateStatus(Page, Convert.ToInt32(Status.Closed), GetId());
	}
}
