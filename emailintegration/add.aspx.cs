...
public partial class EmailIntegration_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_emailintegration", "emailintegrationid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
			...
			gblData.FillDropdown(ddlemailintegrationmoduleid, "tbl_emailintegrationmodule", "emailintegrationmodule_modulename", "emailintegrationmodule_emailintegrationmoduleid", "", "emailintegrationmodule_modulename");
			//FillDropDown_END
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Email Integration";
        }
        else
        {
            lblPageTitle.Text = "Edit Email Integration";
        }
	...
	private void PopulateSubGrid()
	{
		...
		sgrid.Populate(gblData, ltemailkeywordmapping, emailkeywordmapping_param, emailkeywordmapping_setting);
	}
			//PopulateSubGrid_END
    ...
    protected void btnSaveAndView_Click(object sender, EventArgs e)
    {
        if (SaveData(false) > 0)
		{
        	Response.Redirect("~/EmailIntegration/view.aspx");
		}
    }
    private int SaveData(bool isclose)
    {
        ...
        if (id > 0)
        {
			...
			gblData.SaveSubTable(id, gblData, emailkeywordmapping_setting);
			//SaveSubTable_END
            //SaveFile_START
			//SaveFile_END
