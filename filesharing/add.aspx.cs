...
public partial class FileSharing_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_filesharing", "filesharingid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add File Sharing";
        }
        else
        {
            lblPageTitle.Text = "Edit File Sharing";
        }
	...
	private void PopulateSubGrid()
	{
		...
		sgrid.Populate(gblData, ltfilesharingrole, filesharingrole_param, filesharingrole_setting);
	}
			//PopulateSubGrid_END
    ...
    protected void btnSaveAndView_Click(object sender, EventArgs e)
    {
        if (SaveData(false) > 0)
		{
        	Response.Redirect("~/FileSharing/view.aspx");
		}
    }
    private int SaveData(bool isclose)
    {
        ...
        if (id > 0)
        {
			...
			gblData.SaveSubTable(id, gblData, filesharingrole_setting);
			//SaveSubTable_END
            //SaveFile_START
			mfufilesharingdocument.Save(id);
			//SaveFile_END
