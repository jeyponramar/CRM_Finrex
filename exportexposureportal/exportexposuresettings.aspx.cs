...
public partial class exportexposureportal_exportexposuresettings : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_exportexposurealerts", "exportexposurealertsid");
    ...
    private int GetId()
    {
        ...
        string query = "select exportexposurealerts_exportexposurealertsid from tbl_exportexposurealerts where exportexposurealerts_clientid="+clientId;
        ...
        if (dr == null)
        {
            ...
            id = obj.InsertData(hstbl, "tbl_exportexposurealerts");
        }
        else
        {
            id = GlobalUtilities.ConvertToInt(dr["exportexposurealerts_exportexposurealertsid"]);
        }
        return id;
    }
    private void Bind()
    ...
    private void BindOptionalFields()
    {
        string query = @"select * from tbl_columns 
                        ...
                        where columns_isgenerate=1 AND isnull(columns_isenableinedit,0)=1 AND module_modulename IN('Export Order','Forward Contract','PCFC')
        ...
        {
            moduleName = GlobalUtilities.ConvertToString(dttblcol.Rows[i]["module_modulename"]);
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        query = @"delete from tbl_customercustomfields where customercustomfields_clientid=" + clientId + @"
                  and customercustomfields_columnsid in(select columns_columnsid from tbl_columns
                    join tbl_module on module_moduleid=columns_moduleid where module_modulename IN('Export Order','Forward Contract','PCFC'))";
        DbTable.ExecuteQuery(query);
