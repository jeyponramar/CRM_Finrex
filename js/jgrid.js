...
//            }
            ...
            if (this.enablePaging) {
                ...
                this.grid.find(".repeater-header").before("<tr class='trpaging'><td colspan='20'>"+
                                ...
                                + refresh + print + excel + pdf + "</tr></table></td></tr>");
            }
            var hiddenRow = 1;
            var rowId = 1;
            var grid = this.grid;
...
//                }
    ...
    //code for grid row selection
    grid = this.grid;
    var prevRow;
    var reacher = false;
    this.grid.find(".repeater-row,.repeater-alt").live("click", function() {

        var setting = grid.find(".g_setting");
        var isselect = getParam(setting, "isselect");
