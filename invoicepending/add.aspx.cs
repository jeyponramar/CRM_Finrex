...
public partial class InvoicePending_add : System.Web.UI.Page
{
    GlobalData gblData = new GlobalData("tbl_invoicepending", "invoicependingid");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                //Populate_END
                string module = Convert.ToString(gblData._CurrentRow["invoicepending_module"]);
                int mid = GlobalUtilities.ConvertToInt(gblData._CurrentRow["invoicepending_moduleid"]);
                Response.Redirect("~/" + module + "/add.aspx?id=" + mid);
            }
        ...
        if (Request.QueryString["id"] == null)
        {
            lblPageTitle.Text = "Add Invoice Pending";
        }
        else
        {
            lblPageTitle.Text = "Edit Invoice Pending";
        }
    ...
    {
        SaveData(false);
        Response.Redirect("~/InvoicePending/view.aspx");
    }
